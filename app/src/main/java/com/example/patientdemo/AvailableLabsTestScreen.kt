...
import com.example.patientdemo.databinding.ActivityLabTestListBinding
...
class AvailableLabsTestScreen : AppCompatActivity() {
    var labBookingListAdapter: LabTestBookingAdapter? = null
    private lateinit var binding: ActivityLabTestListBinding
    private var searchType: String? = ""

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_lab_test_list)
    ...
    private fun searchLabs(searchEntity: AdvanceSearchEntity) {
        val start = Timestamp("2022-06-06T06:41:36")
        val end = Timestamp("2022-08-18T23:59:59")
        ...
        val speciality = searchEntity.speciality?: "Lab Booking"
        val startDateTime = searchEntity.startTime?: formatDate(Date())
        val endDateTime = searchEntity.endTime?: formatDate(Date(Date().time + 100 * 60 * 60 * 1000))
        ...
        call.enqueue(object : Callback<SearchResponse> {
            override fun onResponse(
                ...
                searchResponse?.let { res ->
                    ...
                    labBookingList?.let {
                        labBookingListAdapter = LabTestBookingAdapter(labBookingList, context = this@AvailableLabsTestScreen)
