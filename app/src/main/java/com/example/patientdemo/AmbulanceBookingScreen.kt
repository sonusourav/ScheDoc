...
import com.example.patientdemo.databinding.ActivityAmbulanceListBinding
...
class AmbulanceBookingScreen : AppCompatActivity() {
    var labBookingListAdapter: AmbulanceBookingAdapter? = null
    private lateinit var binding: ActivityAmbulanceListBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_ambulance_list)
    ...
    private fun searchLabs(searchEntity: AdvanceSearchEntity) {
        ...
        val endDateTime =
            searchEntity.endTime ?: formatDate(Date(Date().time + 100 * 60 * 60 * 1000))
        ...
        call.enqueue(object : Callback<SearchResponse> {
            override fun onResponse(
                ...
                searchResponse?.let { res ->
                    ...
                    Log.d(
                        "sonusourav",
                        "list size ${labBookingList?.size} ${Gson().toJson(labBookingList)}"
                    )
                    labBookingList?.let {
                        labBookingListAdapter = AmbulanceBookingAdapter(
                            labBookingList,
                            context = this@AmbulanceBookingScreen
                        ) { show ->
